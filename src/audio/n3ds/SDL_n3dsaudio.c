...
static inline void contextLock(_THIS) { LightLock_Lock(&this->hidden->lock); }

static inline void contextUnlock(_THIS) {
  LightLock_Unlock(&this->hidden->lock);
}

static void N3DSAUD_LockAudio(_THIS) { contextLock(this); }

static void N3DSAUD_UnlockAudio(_THIS) { contextUnlock(this); }
...
static void audioFrameFinished(void *context) {
  ...
  if (shouldBroadcast) CondVar_Broadcast(&this->hidden->cv);

  contextUnlock(this);
}

static int N3DSAUDIO_OpenDevice(_THIS, void *handle, const char *devname,
                                int iscapture) {
  this->hidden =
  ...
  while ((!valid_datatype) && (test_format)) {
    ...
    switch (test_format) {
      case AUDIO_S8:
        ...
        this->hidden->format = (this->spec.channels == 2)
  ...
  if (this->spec.size >= UINT32_MAX / 2) return (-1);
  ...
  for (i = 0; i < NUM_BUFFERS; i++) {
    ...
    this->hidden->waveBuf[i].nsamples =
        this->hidden->mixlen / this->hidden->bytePerSample;
...
static void N3DSAUDIO_WaitDevice(_THIS) {
  ...
  while (!this->hidden->isCancelled &&
         this->hidden->waveBuf[this->hidden->nextbuf].status !=
             NDSP_WBUF_FREE) {
...
static Uint8 *N3DSAUDIO_GetDeviceBuf(_THIS) { return (this->hidden->mixbuf); }
...
static void N3DSAUDIO_CloseDevice(_THIS) {
  ...
  if (!this->hidden->isCancelled) ndspChnReset(0);
...
static void N3DSAUDIO_ThreadInit(_THIS) {
  ...
  if (current_priority > 0x19)
    current_priority--;
  else
    current_priority = 0x19;  // priority 0x18 is for video thread that is
                              // activated by a signal and than must run at
                              // maximum priority to avoid flickering
  if (current_priority > 0x2F) current_priority = 0x2F;

  svcSetThreadPriority(CUR_THREAD_HANDLE, current_priority);
}

static int N3DSAUDIO_Init(SDL_AudioDriverImpl *impl) {
